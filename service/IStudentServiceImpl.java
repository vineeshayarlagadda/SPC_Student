...
public class IStudentServiceImpl implements IStudentService{
	...
	public Student updateStudent(Student student) throws StudentIDNotFoundException{
		...
		if(s1.isEmpty()) throw new StudentIDNotFoundException(id);
			...
			s.setSubjectIdList(student.getSubjectIdList());

			List<Integer> subjectIdList = student.getSubjectIdList();
			List<Subject> studentSubjectsList = new ArrayList<>();
			for(int i : subjectIdList) {
				Subject subject = subjectRepo.findById(i).get();
				studentSubjectsList.add(subject);
			}
			s.setSubjects(studentSubjectsList);
	...
	@Override
	@Transactional
	public String deleteStudent(long id) throws StudentIDNotFoundException{
